...
typedef struct {
    ...
    // ALL Objects on the screen. It's interesting that we wrap this up into one struct
    lv_obj_t *parent;
    lv_obj_t *current_screen;
    lv_obj_t *title_label;          // Probably could use this as info for simulator
    lv_obj_t *stratagem_label;      // For displaying how many correct stratagems have happened
    lv_obj_t *stratagem_container;  // The space for displaying the arrows
    ...
    uint16_t current_index;         // What arrow are we at
    uint16_t correct_stratagems;    // Format of x / y. 
...
lv_obj_t *attack_create(const attack_config_t *config) {        // Literally we'll call 
...
static void check_input(attack_t *attack, arrow_t arrow) {
    if (attack->current_stratagem[attack->current_index] == arrow) {                    // We succeeded!
        ...
        if (attack->current_index == attack->stratagem_length) {                        // We've reached the end of the stratagem
            attack->correct_stratagems++;                                               // Our attack Number goes up
            attack->current_index = 0;                                                  // Reset back to beginning
            bool continue_attack  = true;                                               // We are gonna keep attack
            if (attack->config.stratagem_callback != NULL) {                            // What do we do when we're done?
                int64_t cb_time = esp_timer_get_time();         
                continue_attack = attack->config.stratagem_callback(                    // KEEP ATTACKING   -   Stratgem_callback will take (battle_id, stratagem_min, stratagem_max, correct_strats, failed, How fast)
            ...
            if (continue_attack) {                                                      // If the results of continue attack is true
                if (attack->correct_stratagems == attack->config.stratagem_count) {     // We've done 8/8 or however many attacks 
                    attack_end(attack, true);                                           // GG
                } else {
                    lv_label_set_text_fmt(attack->stratagem_label, "%d/%d", attack->correct_stratagems + 1,
                                          attack->config.stratagem_count);              // 6/9 or whatever x/x
                    update_stratagem(attack);                                           // Resets the visuals
                }
            } else {
                attack_end(attack, true);
            }
        }
    } else {                                                                            // We Failed an attack
